...
public final class BloomFilterIndex<E> implements BloomIndex<E> {
     ...
     * @param sibling
     */
    private static void merge(final BFINode node, final BFINode sibling, final boolean isRightSibling,
                              final InsDelUpdateStatistics stat) {
    ...
    private static BloomFilter createZeroBloomFilter(BloomFilter filter) {
        ...
        long expectedNumberOfFilterElements = filter.getExpectedNumberOfElements();
        int k = filter.getK();
        int metric = filter.getMetric();

        // create all zeros BloomFilter
        BloomFilter zeroFilter = new BloomFilter(filter.h, bitsPerElement, expectedNumberOfFilterElements, k, metric);
    ...
    public BloomFilter<E> getBloomFilter(int id) {
        return idMap.get(id).value;
    }
    ...
    private void deleteNode(BFINode<E> childNode, InsDelUpdateStatistics stat) {
        ...
        if (!node.needMerge()) {
            ...
            if (index + 1 < node.parent.children.size()) {
                ...
                if (index - 1 < 0) {
                    System.err.println("Error " + this.toString() + " node: " + node.toString() + "childNode: " +
                            childNode.toString());
                    assert false;
                }
     ...
     * @param sibling
     */
    private void redistribute(BFINode<E> node, BFINode<E> sibling, boolean isRightSibling,
     ...
     * @return
     */
    public ArrayList<BloomFilter<E>> searchBloomFilters(E o, SearchStatistics stat) {
     ...
     * @return
     */
    public ArrayList<BloomFilter<E>> findMatches(BFINode<E> node, E o, SearchStatistics stat) {
     ...
     * @return
     */
    public ArrayList<BloomFilter<E>> naivefindMatches(BFINode<E> node, E o, SearchStatistics stat) {
     ...
     */
    private BFINode<E> insertRight(boolean isInBFI, BFINode<E> current, BFINode<E> newChild, BFINode<E> rightmost,
                                   InsDelUpdateStatistics stat) {
    ...
    private BloomFilter<E> aggregateChildren(BFINode<E> node) {
        ...
        if (!node.value.getBitSet().equals(current.getBitSet())) {
            throw new RuntimeException("bug " + node.children.size() + " " + current.getBitSet().cardinality() + " " +
        ...
        if (!current.isLeaf()) {
            ...
            // there was a split;
                ...
                if (current.parent == null) {
                    ...
                    BFINode<E> newRoot =
                            new BFINode<E>(createZeroBloomFilter(current.value), this.order, this.splitFull);
    ...
    private BFINode<E> split(BFINode<E> current, InsDelUpdateStatistics stat) {
        ...
        assert current.children != null && current.children.size() >= 2 * this.order + 1 :
                "Split not needed in split" + current;
    ...
    private BFINode<E> splitRight(BFINode<E> current, BFINode<E> rightmost, InsDelUpdateStatistics stat) {
        ...
        // sanity check: current node should have 2*d +1 children
        assert current.children != null && current.children.size() >= 2 * this.order + 1 :
                "Split not needed in splitRight" + current;
    ...
    private static class BFINode<EL> {
        ...
        public boolean needSplit() {
            ...
            if (splitFull1) {
                ...
                return !(this.children == null || this.children.size() <= 2 * this.order1 || this.value.isFull());
            }
        }
        ...
        public boolean needMerge() {
            return this.parent != null && this.children.size() < this.order1;
        }
        ...
        private int findClosestIndex(ArrayList<BFINode<EL>> nodeList, InsDelUpdateStatistics stat) {
            ...
            for (int i = 1; i < nodeList.size(); i++) {
                ...
                if (currentDistance < minDistance ||
                        (minDistance - currentDistance < 0.00001 && Math.random() < 1.0 / nodeList.size())) {
