...
public final class FlatBloomFilterIndex<E> implements BloomIndex<E> {
        ...
        if (busy.getWord(index / 64) == 0) {
            ...
            for (Map.Entry<Integer, Integer> me : idMap.entrySet()) {
                if (me.getValue().intValue() / 64 >= index / 64) {
                    idMap.put(me.getKey(), me.getValue().intValue() - 64);
                }
            }
        } else {
            clearBloomAt(index);
    ...
    public void insertBloomFilter(BloomFilter<E> bf, InsDelUpdateStatistics stat) {
        if (h != null) {
            if (bf.getHasher() != h) {
                throw new RuntimeException("You are using more than one hasher");
            }
        } else {
            h = bf.getHasher();
        }
    ...
    public List<Integer> search(E o, SearchStatistics stat) {
        ...
        for (int i = 0; i < buffer.size(); ++i) {
            ...
            while (w != 0) {
                long t = w & -w;
                answer.add(fromindextoId.get(i * 64 + Long.bitCount(t - 1)));
                w ^= t;
            }
        }
        return answer;
    ...
    public int updateIndex(BloomFilter<E> newBloomFilter, InsDelUpdateStatistics stat) {
        ...
        setBloomAt(idMap.get(newBloomFilter.getID()), newBloomFilter.getBitSet());
        return 0;
    }
    ...
    public int replaceIndex(BloomFilter<E> newBloomFilter) {
        ...
        replaceBloomAt(idMap.get(newBloomFilter.getID()), newBloomFilter.getBitSet());
        return 0;
    }
