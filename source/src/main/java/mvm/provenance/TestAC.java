...
public class TestAC {
    ...
    public static void main(String[] args) {
        ...
        while (iArgs + 1 < args.length) {
            ...
            if (paramName.equalsIgnoreCase("-falsePositiveProb")) {
                falsePosProb = Double.parseDouble(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-expectedNbElemInBloomFilter")) {
                expectedNbElemInFilter = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-initialNbElemInBloomFilter")) {
                initialNbElemInFilter = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-nbBloomFilters")) {
                nbBFs = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-bloofiOrder")) {
                order = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-constructionMethod")) {
                bulkOrIncremental = args[iArgs + 1];
            } else if (paramName.equalsIgnoreCase("-nbYesSearches")) {
                nbYesSearches = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-nbNoSearches")) {
                nbNoSearches = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-splitAllOneNodesIfOverflow")) {
                splitFull = Boolean.parseBoolean(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-metric")) {
                String metricString = args[iArgs + 1];
                if (metricString.equalsIgnoreCase("Hamming")) {
                    metric = 1;
                } else if (metricString.equalsIgnoreCase("Jaccard")) {
                    metric = 2;
                } else if (metricString.equalsIgnoreCase("Cosine")) {
                    metric = 3;
                } else {
            ...
            } else if (paramName.equalsIgnoreCase("-nbBFInsertsDeletes")) {
                nbInsDel = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-nbUpdates")) {
                nbUpdates = Integer.parseInt(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-nonOverlappingRanges")) {
                nonRandomRanges = Boolean.parseBoolean(args[iArgs + 1]);
            } else if (paramName.equalsIgnoreCase("-nbRuns")) {
                nbRuns = Integer.parseInt(args[iArgs + 1]);
            } else {
        ...
        if (!(expectedNbElemInFilter >= initialNbElemInFilter)) {
            System.err.println(
                    "Expected number of elements in filter should be larger than the initial number of elements in filter");
            printHelp();
            return;
        }
        ...
        for (int runNb = 0; runNb < nbRuns; runNb++) {
            ...
            for (int i = 0; i < nbBFs; i++) {
                ...
                if (nonRandomRanges) {
                    current = createBloomFilterAll(h, falsePosProb, expectedNbElemInFilter, i * initialNbElemInFilter,
                            i * initialNbElemInFilter + nbElemInFilterToConstructTree - 1, metric);
                } else {
                    current = createBloomFilterRandom(h, falsePosProb, expectedNbElemInFilter,
                            nbElemInFilterToConstructTree, metric, insertedValues, nextIndexInArray);
            ...
            */
            ...
            if (!buildIndex) {
                ...
                System.out.print("| Nb Bloom filters |" + nbBFs + "| Nb yes searches| " + nbYesSearches);
            ...
            if ((!Bloofi2) && (!Naive) && bulkOrIncremental.equalsIgnoreCase("b")) {
                ...
                if (!collectStats) {
                    System.out.println(
                            "Start creating the index using the bulk load functionality. This might take a while.");
                }
            ...
            if (collectStats) {
                ...
                System.out.print("| Index height| " + bfi.getHeight() + "| Nb nodes| " + bfi.getSize() +
                        "| Filter size| " + bfi.getBloomFilterSize() + "| IsRootAllOne| " + bfi.getIsRootAllOne() +
                        "| Nb children of root| " + bfi.getNbChildrenRoot() + "| Ins stats: nbBFAccessed| " +
                        insStat.nbBFAccessed + "| Ins stats: nbBFNodesAccessed| " + insStat.nbBFNodesAccessed +
                        "| Ins stats: nbSplits| " + insStat.nbSplits + "| Ins stats: nbMerges| " + insStat.nbMerges +
            ...
            if (initialNbElemInFilter != nbElemInFilterToConstructTree) {
                ...
                for (int i = 0; i < nbBFs; i++) {
                    ...
                    current = bfList.get(i);
                    //add more elements
                    for (int j = i * initialNbElemInFilter + nbElemInFilterToConstructTree;
                         j < (i + 1) * initialNbElemInFilter; j++) {
                        current.add(j);
                    }
                    //update BlooFI
                    startTime = System.currentTimeMillis();
            ...
            System.out.print("| Total BF updates | " + nbBFsUpdated + "| Total update time milis| " +
                    totalUpdateTimeMilis + updateStat.toString());
            ...
            if (!collectStats) {
                System.out.println(
                        "Start searching for " + nbYesSearches + " elements inserted in any of the bloom filters");
            }
            ...
            if (!collectStats) {
                System.out.println(
                        "Start searching for " + nbNoSearches + " elements NOT inserted in any of the bloom filters");
            }
            ...
            for (int i = 0; i < nbInsDel; i++) {
                //do one insert
                if (nonRandomRanges) {
                    current = createBloomFilterAll(h, falsePosProb, expectedNbElemInFilter,
                            nbCreatedBFs * initialNbElemInFilter, (nbCreatedBFs + 1) * initialNbElemInFilter - 1,
                            metric);
                } else {
                    current = createBloomFilterRandom(h, falsePosProb, expectedNbElemInFilter, initialNbElemInFilter,
                            metric, null, 0);
                }
            ...
            System.out.println(
                    "| Total BF inserts | " + insertedBFs + "| Total insert time milis| " + totalInsertTimeMilis +
                            insertStat.toString() + "|Total BF deletes | " + deletedBFs +
                            "| Total delete time milis| " + totalDeleteTimeMilis + deleteStat.toString());
        } //end run experiments
    }

    private static void printHelp() {
        System.out.println("Input parameters: " + " -bloofi | -bloofi2 | -naive" + " -falsePositiveProb falsePosProb" +
                " -expectedNbElemInBloomFilter expectedNbElemInFilter" +
                " -initialNbElemInBloomFilter initialNbElemInFilter" + " -nbBloomFilters  nbBFs" +
                " -bloofiOrder order" + " -constructionMethod b | i (bulk or incrementl)" +
                " -nbYesSearches nbyesSearches" + " -nbNoSearches nbNoSearches" +
                " -splitAllOneNodesIfOverflow true | false" + " -metric Hamming | Jaccard | Cosine" +
                " -nbBFInsertsDeletes nbBloomFiltersInsertsOrDeletes" +
                " -nbUpdates nbOfElementsToBeInsertedDuringUpdateInEachFilter" + " -nonOverlappingRanges true | false" +
                " -nbRuns numberOfRunsForExperiments");
        System.out.println("Ex. -bloofi -falsePositiveProb 0.01 " + " -expectedNbElemInBloomFilter 1000 " +
                " -initialNbElemInBloomFilter 10" + " -nbBloomFilters 1000 " + " -bloofiOrder 4 " +
                " -constructionMethod i" + " -nbRuns 10");
    }
    ...
    private static int searchSomeElements(BloomIndex<Integer> bfi, int startRange, int endRange, int nbSearchesToDo) {
        ...
        System.out.print(" |Min steps| " + minSearchSteps + "| Max steps | " + maxSearchSteps + "| Avg steps | " +
                (double) sumSearchSteps / nbSearches + "| Avg time millis | " + (double) sumSearchTime / nbSearches +
                "| Total time millis | " + sumSearchTime + "| Nb searches | " + nbSearches);
     ...
     * @return number of searches that returned some result
     */
    private static int searchSomeElements(BloomIndex<Integer> bfi, int[] values, int nbSearchesToDo) {
        ...
        System.out.print(" |Min steps| " + minSearchSteps + "| Max steps | " + maxSearchSteps + "| Avg steps | " +
                (double) sumSearchSteps / nbSearches + "| Avg time millis | " + (double) sumSearchTime / nbSearches +
                "| Total time millis | " + sumSearchTime + "| Nb searches | " + nbSearches + "| Nb results | " +
                nbResults);
     ...
     * @return number of searches that returned some result
     */
    private static int searchSomeElementsInBFList(List<BloomFilter<Integer>> bfList, int startRange, int endRange,
                                                  int nbSearchesToDo) {
        ...
        System.out.print(" |Min steps| " + minSearchSteps + "| Max steps | " + maxSearchSteps + "| Avg steps | " +
                (double) sumSearchSteps / nbSearches + "| Avg time millis | " + (double) sumSearchTime / nbSearches +
                "| Total time millis | " + sumSearchTime + "| Nb searches | " + nbSearches + " |Min steps to first| " +
                minSearchStepsToFirst + "| Max steps to first| " + maxSearchStepsToFirst + "| Avg steps to first| " +
                (double) sumSearchStepsToFirst / nbSearches + "| Avg time millis to first| " +
                (double) sumSearchTimeToFirst / nbSearches + "| Total time millis to first| " + sumSearchTimeToFirst);
     ...
     * @return number of searches that returned some result
     */
    private static int searchSomeElementsInBFList(List<BloomFilter<Integer>> bfList, int[] values, int nbSearchesToDo) {
        ...
        System.out.print(" |Min steps| " + minSearchSteps + "| Max steps | " + maxSearchSteps + "| Avg steps | " +
                (double) sumSearchSteps / nbSearches + "| Avg time millis | " + (double) sumSearchTime / nbSearches +
                "| Total time millis | " + sumSearchTime + "| Nb searches | " + nbSearches + "| Nb results | " +
                nbResults + " |Min steps to first| " + minSearchStepsToFirst + "| Max steps to first| " +
                maxSearchStepsToFirst + "| Avg steps to first| " + (double) sumSearchStepsToFirst / nbSearches +
     ...
     * @return
     */
    public static ArrayList<BloomFilter<Integer>> searchAllInBFList(List<BloomFilter<Integer>> bfList, Integer o,
     ...
     * @return
     */
    public static ArrayList<BloomFilter<Integer>> searchFirstInBFList(List<BloomFilter<Integer>> bfList, Integer o,
                                                                      SearchStatistics stat) {
     ...
     * @return
     */
    private static BloomFilter<Integer> createBloomFilterAll(Hasher h, double falsePosProb, int expectedNbElemInFilter,
                                                             int start, int end, int metric) {
     ...
     * @return
     */
    private static BloomFilter<Integer> createBloomFilterRandom(Hasher h, double falsePosProb,
                                                                int expectedNbElemInFilter, int actualNbElemInFilter,
                                                                int metric, int[] values, int nextIndexInArray) {
        //create the empty bloom filter
        BloomFilter<Integer> bf = new BloomFilter<Integer>(h, falsePosProb, expectedNbElemInFilter, metric);
