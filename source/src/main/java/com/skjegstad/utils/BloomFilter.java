...
public final class BloomFilter<E> implements Serializable {
    ...
    static int hashSeed = 137953731;
    private static int lastID = 0;
    public Hasher h;
    private BitSet bitset;
    private int bitSetSize;
    private double bitsPerElement;
    private long expectedNumberOfFilterElements; // expected (maximum) number
     ...
     *                filter.
     */
    public BloomFilter(double falsePositiveProbability, long expectedNumberOfElements, int metric) {
     ...
     *                filter.
     */
    public BloomFilter(final Hasher hash, double falsePositiveProbability, long expectedNumberOfElements, int metric) {
        this(hash, Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))) / Math.log(2), // c = k / ln(2)
                expectedNumberOfElements, (int) Math.ceil(-(Math.log(falsePositiveProbability) / Math.log(2))),
                metric); // k = ceil(-log_2(false prob.))
    }
     ...
     */
    public BloomFilter(final Hasher hash, final double c, final long n, final int k, final int metric) {
        this.h = hash;
        ...
        Report.INSTANCE.debug("BitSet-Size-1=%d,expectedNumberOfFilterElements=%d,k=%d,c=%d", bitSetSize, n, k, c);
     ...
     *                is the number of hash functions used.
     */
    public BloomFilter(final double c, final long n, final int k, final int metric) {
        ...
        this.bitsPerElement = c;
        this.bitSetSize = (int) Math.ceil(c * n);
        this.h = new Hasher(hashSeed, bitSetSize);
        this.h.setNumberOfRandomKeys(k);
        numberOfAddedElements = 0;
        Report.INSTANCE.debug("BitSet-Size-2=%d,expectedNumberOfFilterElements=%d,k=%d,c=%f", bitSetSize, n, k, c);
        this.bitset = new BitSet(bitSetSize);
        // AC: add an ID for every BloomFilter created
        lastID++;
        this.id = lastID;
     ...
     *                expected to contain.
     */
    public BloomFilter(final Hasher hash, int bitSetSize, int expectedNumberOElements, int metric) {
        this(hash, bitSetSize / (double) expectedNumberOElements, expectedNumberOElements,
                (int) Math.round((bitSetSize / (double) expectedNumberOElements) * Math.log(2.0)), metric);
     ...
     */
    public BloomFilter(final Hasher hash, int bitSetSize, int expectedNumberOfFilterElements,
    ...
    public int hashCode() {
        ...
        hash = 61 * hash + 123456; //+ this.expectedNumberOfFilterElements;
    ...
    public void orBloomFilter(BloomFilter<E> filter) {
        ...
        assert bitSetSize == filter.size() :
                "Different size bitsets in orBloomFIlter: " + bitSetSize + " and " + filter.size();
    ...
    public String toString() {
        return "ID:" + id + ":" + bitset.cardinality() + ":" + bitSetSize; // + ":" + bitset.toString();
    }
    // missing
    ...
    private double computeCosineDistance(BloomFilter<E> filter) {
        ...
        assert bitSetSize == filter.size() :
                "Different size bitsets in computeCosineDistance: " + bitSetSize + " and " + filter.size();
        ...
        if (count1 > 0 || count2 > 0) {
            distance = 1.0 - countAND / (Math.sqrt(count1) * Math.sqrt(count2));
        }
        return distance;
    }
    ...
    private int computeHammingDistance(BloomFilter<E> filter) {
        ...
        assert bitSetSize == filter.size() :
                "Different size bitsets in computeHammingDistance: " + bitSetSize + " and " + filter.size();
    ...
    private double computeJaccardDistance(BloomFilter<E> filter) {
        ...
        assert bitSetSize == filter.size() :
                "Different size bitsets in computeJaccardDistance: " + bitSetSize + " and " + filter.size();
