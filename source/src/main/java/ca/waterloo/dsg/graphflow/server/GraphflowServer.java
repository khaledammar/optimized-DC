...
public class GraphflowServer {
    ...
    public void start() throws IOException {
        ...
        grpcServer = ServerBuilder.forPort(GRPC_PORT).addService(new GraphflowQueryImpl()).build().start();
        System.err.println("*** gRPC server running.");

        System.err.println("*** starting http server...");
        final PlanViewerHttpServer planViewerHttpServer = new PlanViewerHttpServer(GRPC_HOST, GRPC_PORT);
    ...
    private class GraphflowQueryImpl extends GraphflowServerQueryGrpc.GraphflowServerQueryImplBase {
        ...
        public void executeQuery(ServerQueryString request, StreamObserver<ServerQueryResult> responseObserver) {
            ...
            try {
                ...
                logger.error(
                        "Unknown error when executing the query '" + request.getMessage() + "'. Exception stack trace:",
                        e);
