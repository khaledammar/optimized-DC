...
public abstract class ParallelArraySerDeUtils {
     ...
     */
    protected abstract void serializeArrayCell(ObjectOutputStream objectOutputStream, int index) throws IOException;
    ...
    public void startSerialization() throws IOException {
        ...
        for (int i = 0; i < numBlocks; i++) {
            ...
            int endIndexOfBlock =
                    ((i + 1) == numBlocks) ? getArraySize() - 1 : (((i + 1) * numArrayIndicesPerFile) - 1);
            final int blockIndex = i;
            Thread serializerThread = new Thread(() -> serializeBlock(blockIndex, startIndexOfBlock, endIndexOfBlock));
    ...
    private void serializeBlock(int blockId, int startIndexOfBlock, int endIndexOfBlock) {
        ...
        try {
            ObjectOutputStream objectOutputStream = IOUtils.constructObjectOutputStream(
                    SerDeUtils.getArrayBlockFilePath(directoryPath, getClassName(), getArrayName(), blockId));
            ...
            objectOutputStream.close();
            logger.info(
                    String.format("%s block %s (from %s to %s) of %s serialized in %.3f ms.", getArrayName(), blockId,
                            startIndexOfBlock, endIndexOfBlock, getClassName(),
                            IOUtils.getElapsedTimeInMillis(blockBeginTimeInNano)));
        } catch (IOException e) {
            logger.error(String.format("Error serializing block %s of %s", blockId, getArrayName()), e);
    ...
    private void deserializeBlock(int blockId) {
        ...
        try {
            ...
            objectInputStream.close();
            logger.info(
                    String.format("%s block %s (from %s to %s) of %s deserialized in %.3f ms.", getArrayName(), blockId,
                            startIndexOfBlock, endIndexOfBlock, getClassName(),
                            IOUtils.getElapsedTimeInMillis(blockBeginTimeInNano)));
        } catch (IOException | ClassNotFoundException e) {
            logger.error(String.format("Error deserializing block %s of %s", blockId, getArrayName()), e);
