...
public class TypeAndPropertyKeyStore implements GraphflowSerializable {
    ...
    public String mapShortToStringType(Short shortType) {
        if (null == shortType || 0 > shortType) {
            throw new NoSuchTypeException("The short " + shortType + " is not present in the " + "key store.");
        }
    ...
    public String mapShortPropertyKeyToString(Short shortPropertyKey) {
        if (null == shortPropertyKey || 0 > shortPropertyKey) {
            throw new NoSuchPropertyKeyException(
                    "The short " + shortPropertyKey + " is not " + "present in the key store.");
        }
    ...
    public Map<Short, Pair<DataType, String>> mapStringPropertiesToShortAndDataTypeOrInsert(
        ...
        return mapStringPropertiesToShortAndDataType(stringProperties, true /* insert if key doesn't exist */, true /* assert all keys exist. Argument not used since a key is inserted if it doesn't
            exist */);
    }
     ...
     */
    public void assertExistingKeyDataTypesMatchPreviousDeclarations(Map<String, Pair<String, String>> properties) {
        mapStringPropertiesToShortAndDataType(properties, false /* do not insert if key doesn't exist */,
    ...
    private Map<Short, Pair<DataType, String>> mapStringPropertiesToShortAndDataType(
        ...
        for (String stringKey : stringProperties.keySet()) {
            ...
            keyDataTypePair = mapStringPropertyKeyValueToShortAndDataType(stringKey,
                    stringDataTypeValuePair.a/* DataType as String */, insertIfKeyDoesntExist, assertAllKeysExist);
            resultProperties.put(keyDataTypePair.a /* key as short */,
                    new Pair<>(keyDataTypePair.b/* DataType */, stringDataTypeValuePair.b/* value as String */));
        }
        return resultProperties;
    }

    @VisibleForTesting
    Pair<Short, DataType> mapStringPropertyKeyValueToShortAndDataType(String stringKey, String stringDataType,
                                                                      boolean insertIfKeyDoesntExist,
                                                                      boolean assertKeyExist) {
        ...
        if (null != key) {
            ...
            if (dataTypeStored != dataType) {
                throw new IncorrectDataTypeException(
                        "Incorrect DataType usage - property key " + stringKey + " has been declared as " +
                                dataTypeStored + " previously but " + "now it used as " + stringDataType.toUpperCase() +
                                ".");
            }
    ...
    @Override
    public void deserializeAll(String inputDirectoryPath) throws IOException, ClassNotFoundException {
