...
public class IntQueue {
    ...
    public void enqueue(int element) {
        ...
        try {
            ...
            System.out.println(
                    String.format("size=%s, capacity=%s, nextItemIndex=%s, queue" + ".length=%s", size, capacity,
                            nextItemIndex, queue.length));
            System.exit(0);
        }
    ...
    public int dequeue() throws NoSuchElementException {
        if (size == 0) {
            throw new NoSuchElementException("IntQueue is empty. Cannot dequeue from an empty" + " IntQueue.");
        }
    ...
    public int peekNext() throws NoSuchElementException {
        if (size == 0) {
            throw new NoSuchElementException(
                    "IntQueue is empty. Cannot peek the first " + "element of an empty IntQueue.");
        }
        return queue[firstItemIndex];
    }

    /**
     * Returns the value at the tail of the queue if one exists.
     *
     * @return int Value at the tail of the queue.
     */
    public int peekLast() throws NoSuchElementException {
        if (size == 0) {
            throw new NoSuchElementException(
                    "IntQueue is empty. Cannot peek the last " + "element of an empty IntQueue.");
        }
        int index = nextItemIndex == 0 ? capacity - 1 : nextItemIndex - 1;
        return queue[index];
    }
    ...
    public String toString() {
        ...
        builder.append("[");
        int length = (firstItemIndex < nextItemIndex) ? nextItemIndex : firstItemIndex + size - nextItemIndex;
