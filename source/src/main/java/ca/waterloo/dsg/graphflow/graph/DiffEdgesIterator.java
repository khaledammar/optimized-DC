...
public class DiffEdgesIterator implements Iterator<int[]> {
     ...
     */
    public DiffEdgesIterator(List<int[]> diffEdges, ShortArrayList diffEdgeTypes, ShortArrayList vertexTypes,
    ...
    private void setIndexToNextEdge() {
        ...
        while (next < diffEdges.size()) {
            ...
            if ((TypeAndPropertyKeyStore.ANY == fromVertexTypeFilter ||
                    ...
                    (TypeAndPropertyKeyStore.ANY == edgeTypeFilter || diffEdgeTypes.get(next) == edgeTypeFilter)) {
                return;
            }
            next++;
        }
