...
public class Min extends AbstractLongDoubleAggregator {
    ...
    protected void aggregateInt(int index, int intValue) {
        this.longValues = ArrayUtils
                .resizeIfNecessary(longValues, index + 1, Long.MAX_VALUE /* value to fill new cells if resizing */);
        this.longValues[index] = Long.min(this.longValues[index], intValue);
    }

    @Override
    protected void aggregateDouble(int index, double doubleValue) {
        this.doubleValues = ArrayUtils
                .resizeIfNecessary(doubleValues, index + 1, Double.MAX_VALUE /* value to fill new cells if resizing */);
