...
public class GraphflowClient {
    ...
    protected String queryServer(String query) {
        ...
        switch (this.getClass().getSimpleName()) {
            case "GraphflowCli":
                request = ServerQueryString.newBuilder().setMessage(query).setReturnType(ReturnType.TEXT).build();
                break;
            case "PlanViewerHttpServer":
                request = ServerQueryString.newBuilder().setMessage(query).setReturnType(ReturnType.JSON).build();
                break;
            default:
