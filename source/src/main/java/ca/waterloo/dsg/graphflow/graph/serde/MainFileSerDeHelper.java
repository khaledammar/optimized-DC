...
public class MainFileSerDeHelper {
     ...
     */
    public static void serialize(GraphflowSerializable graphflowSerializable, String outputDirectoryPath)
            throws IOException {
        long beginTimeInNano = System.nanoTime();
        logger.info(String.format("Serializing main file of %s.", graphflowSerializable.getMainFileNamePrefix()));
        ObjectOutputStream objectOutputStream = IOUtils.constructObjectOutputStream(
                SerDeUtils.getMainFilePath(outputDirectoryPath, graphflowSerializable.getMainFileNamePrefix()));
        graphflowSerializable.serializeMainFile(objectOutputStream);
        objectOutputStream.close();
        logger.info(
                String.format("Main file of %s serialized in %.3f ms.", graphflowSerializable.getMainFileNamePrefix(),
     ...
     */
    public static void deserialize(GraphflowSerializable graphflowSerializable, String inputDirectoryPath)
            throws IOException, ClassNotFoundException {
        long beginTimeInNano = System.nanoTime();
        logger.info(String.format("Deserializing %s.", graphflowSerializable.getMainFileNamePrefix()));
        ObjectInputStream objectInputStream = IOUtils.constructObjectInputStream(
                SerDeUtils.getMainFilePath(inputDirectoryPath, graphflowSerializable.getMainFileNamePrefix()));
        graphflowSerializable.deserializeMainFile(objectInputStream);
        objectInputStream.close();
        logger.info(
                String.format("Main file of %s deserialized in %.3f ms.", graphflowSerializable.getMainFileNamePrefix(),
                        IOUtils.getElapsedTimeInMillis(beginTimeInNano)));
    }
}
