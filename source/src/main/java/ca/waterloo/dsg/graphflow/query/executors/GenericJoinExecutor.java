...
public class GenericJoinExecutor {
    ...
    public GenericJoinExecutor(List<List<GenericJoinIntersectionRule>> stages, Map<String, Integer> variableIndicesMap,
    ...
    public static boolean hasSameStages(GenericJoinExecutor a, GenericJoinExecutor b) {
        ...
        for (int i = 0; i < a.stages.size(); i++) {
            ...
            for (int j = 0; j < a.stages.get(i).size(); j++) {
                if (!GenericJoinIntersectionRule.isSameAs(a.stages.get(i).get(j), b.stages.get(i).get(j))) {
                    return false;
                }
            }
        }
    ...
    public void execute() {
        ...
        Iterator<int[]> iterator = graph.getEdgesIterator(firstGJIntersectionRule.getGraphVersion(),
                firstGJIntersectionRule.getDirection(), firstGJIntersectionRule.getFromVertexTypeFilter(),
                firstGJIntersectionRule.getToVertexTypeFilter(), firstGJIntersectionRule.getEdgeTypeFilter());
        if (!iterator.hasNext()) {
        ...
        while (iterator.hasNext()) {
            ...
            for (int i = 1; i < stages.get(0).size(); i++) {
                ...
                if (!graph.isEdgePresent(prefix[0], prefix[1], rule.getDirection(), rule.getGraphVersion(),
    ...
    private void extend(int[][] prefixes, int stageIndex) {
        ...
        List<GenericJoinIntersectionRule> genericJoinIntersectionRules = this.stages.get(stageIndex);
        ...
        for (int[] prefix : prefixes) {
            ...
            GenericJoinIntersectionRule minCountRule = getMinCountIndex(prefix, genericJoinIntersectionRules);
            ...
            IntArrayList extensions = this.graph
                    .getSortedAdjacencyList(prefix[minCountRule.getPrefixIndex()], minCountRule.getDirection(),
                            minCountRule.getGraphVersion())
                    .getFilteredNeighbourIds(minCountRule.getToVertexTypeFilter(), minCountRule.getEdgeTypeFilter(),
                            this.graph.getVertexTypes());
            ...
            for (GenericJoinIntersectionRule rule : genericJoinIntersectionRules) {
                ...
                extensions = this.graph.getSortedAdjacencyList(prefix[rule.getPrefixIndex()], rule.getDirection(),
                        rule.getGraphVersion()).getIntersection(extensions, rule.getEdgeTypeFilter());
            }
    ...
    private GenericJoinIntersectionRule getMinCountIndex(int[] prefix,
        ...
        for (GenericJoinIntersectionRule rule : genericJoinIntersectionRules) {
            int extensionCount = this.graph
                    .getSortedAdjacencyList(prefix[rule.getPrefixIndex()], rule.getDirection(), rule.getGraphVersion())
                    .getSize();
