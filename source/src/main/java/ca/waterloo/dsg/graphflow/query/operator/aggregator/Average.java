...
public class Average extends AbstractLongDoubleAggregator {
    ...
    protected void aggregateInt(int index, int intValue) {
        ...
        this.longValues =
                ArrayUtils.resizeIfNecessary(this.longValues, index + 1, 0 /* value to fill new cells if resizing */);
    ...
    protected void aggregateDouble(int index, double doubleValue) {
        adjustSizeAndIncrementCount(index);
        this.doubleValues =
                ArrayUtils.resizeIfNecessary(doubleValues, index + 1, 0.0 /* value to fill new cells if resizing */);
        this.doubleValues[index] += doubleValue;
    }

    private void adjustSizeAndIncrementCount(int index) {
        this.counts = ArrayUtils.resizeIfNecessary(this.counts, index + 1, 0 /* value to fill new cells if resizing */);
        this.counts[index] += 1;
    }
