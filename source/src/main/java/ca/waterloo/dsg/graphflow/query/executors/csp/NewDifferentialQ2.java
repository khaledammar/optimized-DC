 ...
 */
public class NewDifferentialQ2 extends NewUnidirectionalUnweightedDifferentialBFS {
    /**
    ...
    static short HASMEMBER = TypeAndPropertyKeyStore.getInstance().mapStringTypeToShort("hasMember");
    public NewDifferentialQ2(int queryId, int source, Direction direction, boolean backtrack, Queries queryType) {
        super(queryId, source, -1, direction, backtrack, queryType);
    }

    public NewDifferentialQ2(int queryId, int source, Direction direction, boolean backtrack, DropIndex dropIndex,
    ...
    static boolean staticGoodNeighbour(short edgeType, short iterationNo){
        if(iterationNo == 1 && edgeType == HASMEMBER)
            return true;
        else if (iterationNo != 1 && edgeType == KNOWS)
            return true;
        else
            return false;
    }
