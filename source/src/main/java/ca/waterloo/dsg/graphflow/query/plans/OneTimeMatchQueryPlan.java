    ...
    public JsonObject toJson() {
        ...
        for (int i = 0; i < stages.size(); ++i) {
            ...
            for (int j = 0; j < stages.get(i).size(); ++j) {
                ...
                ruleJson.addProperty(JsonUtils.VARIABLE, orderedVariables.get(rule.getPrefixIndex()));
                ...
                if (TypeAndPropertyKeyStore.ANY != edgeTypeFilter) {
                    ruleJson.addProperty(JsonUtils.EDGE_TYPE,
                ...
                if (TypeAndPropertyKeyStore.ANY != fromVertexTypeFilter) {
                    ruleJson.addProperty(JsonUtils.FROM_VERTEX_TYPE,
                            TypeAndPropertyKeyStore.getInstance().mapShortToStringType(fromVertexTypeFilter));
                }
                Short toVertexTypeFilter = rule.getToVertexTypeFilter();
                if (TypeAndPropertyKeyStore.ANY != toVertexTypeFilter) {
                    ruleJson.addProperty(JsonUtils.TO_VERTEX_TYPE,
                            TypeAndPropertyKeyStore.getInstance().mapShortToStringType(toVertexTypeFilter));
                }
