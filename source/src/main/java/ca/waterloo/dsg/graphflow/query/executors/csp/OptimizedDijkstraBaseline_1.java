...
public class OptimizedDijkstraBaseline_1 {

    private static final OptimizedDijkstraBaseline_1 INSTANCE = new OptimizedDijkstraBaseline_1();

    private static final Logger logger = LogManager.getLogger(OptimizedDijkstraBaseline_1.class);
    ...
    private OptimizedDijkstraBaseline_1() {
    }

    public static OptimizedDijkstraBaseline_1 getInstance() {
        return INSTANCE;
    }
    ...
    public long execute(int source, int destination, boolean is_weighted) {
        ...
        //        Queue<Integer> pqueue = new PriorityQueue<>(Graph.getInstance().getVertexCount());
        PriorityQueue<Integer> pqueue = new PriorityQueue<>(Graph.getInstance().getVertexCount(),
                //  PriorityQueue<Integer> pqueue = new PriorityQueue<>(20,
                new NodeDistanceComparator());
        //NodeDegreeComparator  -- 367.73577 ms.
        //NodeDegreeComparator -- 337.889339 ms.
    ...
    private class NodeDegreeComparator implements Comparator<Integer> {

        @Override
        public int compare(Integer x, Integer y) {

            int x_degree = Graph.INSTANCE.getVertexFWDDegree(x);
            int y_degree = Graph.INSTANCE.getVertexFWDDegree(y);

            //System.out.println("FWD degree ("+x+") = "+x_degree+"     -      Y= "+y_degree);
