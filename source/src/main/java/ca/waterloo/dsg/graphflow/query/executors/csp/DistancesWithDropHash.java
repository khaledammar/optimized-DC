...
import ca.waterloo.dsg.graphflow.query.executors.csp.DistancesWithDropBloom.DropType;
 ...
 */
public class DistancesWithDropHash extends Distances {
     ...
     */
    public DistancesWithDropHash(int queryId, int source, int destination, float prob, NewUnidirectionalDifferentialBFS.Queries queryType) {
     ...
     * Default public constructor.
     */
    public DistancesWithDropHash(int queryId, int source, int destination, NewUnidirectionalDifferentialBFS.Queries queryType) {
        super(queryId, source, destination, queryType); // includes a call to init Dropped and Real Diff
        skipDiff = 0;
        dropRandomGenerator = new Random(generatorSeed);
        dropProbability = 0;
    ...
    public DistancesWithDropHash(int queryId, int source, int destination, Graph.Direction d, float prob
    ...
    public DistancesWithDropHash(int queryId, int source, int destination, Graph.Direction d, float prob,
                                 DropType dropType, String type, int minimumDegree, int maximumDegree,
                                 NewUnidirectionalDifferentialBFS.Queries queryType) {
...
*/
    ...
    public DistancesWithDropHash(int queryId, int source, int destination, Graph.Direction d,
                                 NewUnidirectionalDifferentialBFS.Queries queryType) {
        super(queryId, source, destination, d, queryType); // includes a call to init Dropped and Real Diff
        skipDiff = 0;
        dropRandomGenerator = new Random(generatorSeed);
...
*/
    ...
    void initializeDroppedDiff(String type) {
        if (!droppedDiffsReady) {
            if (type.equals("Iteration")) {
                Report.INSTANCE.debug("We do not support iteration Hash.");
                System.exit(1);
            } else {
                //Report.INSTANCE.debug("Query Hash");
                droppedDiffs = new MultiQueryDiffHashPerQueryPrimitive(numQueries);
            }
    ...
    */
    ...
    protected void removeVertexDistance(int vertexId, short iterationNo) {
        ...
        if (droppedDiffs.checkVertexIteration(queryId, vertexId, iterationNo, true)) {
    ...
    void setVertexDistance(int vertexId, short iterationNo, long distance) {
         ...
         */
        boolean vertex_found_dropped = droppedDiffs.checkVertexIteration(queryId, vertexId, iterationNo, true);
        ...
        while (indexToAdd < limit) {
            ...
            if (iter == iterationNo) {
                if (debug(vertexId)) {
                    Report.INSTANCE.error("***** Vertex " + vertexId + " with iteration " + iterationNo + " is found? " +
                            true);
                }
        ...
        if (debug(vertexId)) {
            Report.INSTANCE.error("---- Distances of v=" + vertexId + " = " +
                    Arrays.toString(getMergedDiffs(vertexId)));
    ...
    void setLatestIterationNumber(short iterationNo) {
        ...
        frontier.clear();
        nextFrontierSize = 0;
        // get all vertices that have diffs - this already remove deletes
    ...
    public Set<Integer> getVerticesWithDiff() {
        ...
        var vertices = droppedDiffs.getVerticesWithDiff(queryId);
        if (vertices != null) {
            allV.addAll(vertices);
        }
        return allV;
    }
     ...
     * @return
     */
    List<Short> getAllIterations(int vertexId, short[] distances) {
    ...
    long getDestinationDistance(short iterationNo, boolean newBatches) {
        ...
        if (newBatches) {
            ...
            if (DistancesWithDropHash.debug(vertexId)) {
                Report.INSTANCE.error("NewBatches Diff: v= " + vertexId + " distances = " +
                        Arrays.toString(distances));
            }
        } else {
            distances = this.getOldDiffs(vertexId); //vIterDistPairMap.get(vertexId);
            if (DistancesWithDropHash.debug(vertexId)) {
                Report.INSTANCE.error("OldBatches Diff: v= " + vertexId + " distances = " +
                        Arrays.toString(distances));
            }
        }
        ...
        if (distances == null) {
            ...
            if (DistancesWithDropHash.debug(vertexId)) {
                Report.INSTANCE
                        .error("** FAKE v = " + vertexId + " distances = " + Arrays.toString(distances) +
 ...
 */
    long getNewDistance(int vertexId, short iterationNo,
        ...
        if (DistancesWithDropHash.debug(vertexId)) {
            Report.INSTANCE
                    .error("----1-- Hash getNewDistance - v= " + vertexId + " i= " + iterationNo + " newBatch? " +
                            newBatches);
        }
        ...
        for (int i = iterationNo; i >= 0; i--) {
            if (highestIter == (short) i) {
                ...
                if (droppedDiffs.checkVertexIteration(queryId, vertexId, (short) i, newBatches)) {
    ...
    boolean isDiffExist(int q, int vertex, short iteration) {
        //return (droppedDiffs.checkVertexIteration(q,vertex,iteration) || super.isDiffExist(q,vertex,iteration));
    ...
    public void mergeDeltaDiffs() {
        super.mergeDeltaDiffs();
        droppedDiffs.removeIterationPermenant(queryId);
    }
    ...
    long getDistance(int vertexId, short iterationNo,
        ...
        if (DistancesWithDropHash.debug(vertexId)) {
            Report.INSTANCE.error("----1-- Hash getDistance - v= " + vertexId + " i= " + iterationNo + " newBatch? " +
                    newBatches);
        }
