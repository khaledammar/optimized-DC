...
public class QueryRelation implements AbstractStructuredQuery {
    ...
    // TODO(semih): This is part of a quick hack to support weighted edges as
    // part of the Graph and SortedAdjacencyLists instead of EdgeStore.
    // This should not be integrated into the master branch.
    ...
    public static boolean isSameAs(QueryRelation a, QueryRelation b) {
        ...
        if (null == a.relationProperties && null == b.relationProperties) {
            return true;
        } else if ((null != a.relationProperties && null == b.relationProperties) || (null == a.relationProperties) ||
                (a.relationProperties.size() != b.relationProperties.size())) {
            return false;
        }
        for (String key : a.relationProperties.keySet()) {
            if (!a.relationProperties.get(key).b.equals((b.relationProperties.get(key).b)) ||
                    !a.relationProperties.get(key).a.toUpperCase()
                            .equals((b.relationProperties.get(key).a.toUpperCase()))) {
                return false;
            }
        }
