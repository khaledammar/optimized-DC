...
public class QueryGraph {
        ...
        if (null != queryRelation.getRelationName()) {
            if (relationNameRelationMap.containsKey(queryRelation.getRelationName())) {
                throw new MalformedMatchQueryException(
                        "Duplicate edge variable: " + queryRelation.getRelationName() + " is defined in the query.");
            }
     ...
     */
    private void addRelationToQueryGraph(String fromVariable, String toVariable, QueryRelation queryRelation) {
    ...
    public int getNumberOfAdjacentRelations(String variable) {
        ...
        return queryGraph.get(variable).keySet().stream()
                .mapToInt(neighbourVariable -> queryGraph.get(variable).get(neighbourVariable).size()).sum();
    }
    ...
    public boolean containsRelation(String variable1, String variable2) {
        return queryGraph.containsKey(variable1) && queryGraph.get(variable1).containsKey(variable2);
    }
    ...
    public List<QueryRelation> getAdjacentRelations(String fromVariable, String toVariable) {
        ...
        if (!queryGraph.get(fromVariable).containsKey(toVariable)) {
            throw new NoSuchElementException(
                    "The neighbour variable '" + toVariable + "' is not " + "present in the adjacency list of '" +
                            fromVariable + "'.");
        }
