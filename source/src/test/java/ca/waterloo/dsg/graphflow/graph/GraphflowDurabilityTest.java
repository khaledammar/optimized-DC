...
public class GraphflowDurabilityTest {
    ...
    public void testSavingAndLoadingSimpleGraph() throws Exception {
        String query =
                "CREATE " + "(1:Person{ name: 'Olivier' })-[:FOLLOWS{ date:3 }]->(2:Person{ name: 'Mohannad'})," +
        ...
        ((CreateQueryPlan) new CreateQueryPlanner(structuredQuery).plan())
                .execute(Graph.getInstance(), new InMemoryOutputSink());
        ...
        Assert.assertTrue(
                TypeAndPropertyKeyStore.isSameAs(oldTypeAndPropertyKeyStore, TypeAndPropertyKeyStore.getInstance()));
    }
}
