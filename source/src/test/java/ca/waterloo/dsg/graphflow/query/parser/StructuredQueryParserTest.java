...
public class StructuredQueryParserTest {
    ...
    public void testParseTriangleMatchQuery() throws Exception {
        ...
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("a"), new QueryVariable("b")));
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("b"), new QueryVariable("c")));
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("c"), new QueryVariable("a")));
    ...
    public void testParseCreateEdgesQuery() throws Exception {
        StructuredQuery actualStructuredQuery;
        String query = "CREATE (1:Person { name: 'Olivier' })-[:FOLLOWS]->(2:Person " +
                "{ name: 'Mohannad' }), (2:Person { name: 'Mohannad' } )-[:FOLLOWS]->" +
                "(3:Person { name: 'Sid' }), (1:Person { name: 'Olivier'})-[:FOLLOWS]->" +
                "(3:Person { name: 'Sid' });";
        try {
            actualStructuredQuery = new StructuredQueryParser().parse(query);
        } catch (ParseCancellationException e) {
    ...
    public void testParseCreateVerticesQuery() throws Exception {
        StructuredQuery actualStructuredQuery;
        String query = "CREATE (1:Person { name: 'Olivier' }), (2:Person " +
                "{ name: 'Mohannad' }), (3:Person { name: 'Sid' })";
        try {
            actualStructuredQuery = new StructuredQueryParser().parse(query);
        } catch (ParseCancellationException e) {
            throw new Exception("ERROR parsing: " + e.getMessage());
    ...
    public void testParseDeleteQuery() throws Exception {
        ...
        StructuredQuery expectedStructuredQuery = new StructuredQuery();
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("1"), new QueryVariable("2")));
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("2"), new QueryVariable("3")));
    ...
    public void parseSimpleShortestPathQuery() throws Exception {
        ...
        StructuredQuery expectedStructuredQuery = new StructuredQuery();
        expectedStructuredQuery.addRelation(new QueryRelation(new QueryVariable("0"), new QueryVariable("9")));
