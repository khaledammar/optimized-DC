...
public class GraphTest {
    ...
    public void testInsertions() throws Exception {
        // Create a graph.
        int[][] edges =
                {{0, 6} /* edgeId: 0 */, {0, 3} /* edgeId: 1 */, {0, 5} /* edgeId: 2 */, {0, 1} /* edgeId: 3 */, {0, 4}
                        /* edgeId: 4 */, {0, 2} /* edgeId: 5 */, {5, 6} /* edgeId: 6 */, {1, 6} /* edgeId: 7 */, {4, 6}
                        /* edgeId: 256 + 0 */, {3, 6} /* edgeId: 256 + 1 */, {2, 6} /* edgeId: 256 + 2 */};
        short[] edgeTypes = {6, 3, 5, 1, 4, 2, 6, 6, 6, 6, 6};
        short[][] vertexTypes =
                {{0, 12}, {0, 6}, {0, 10}, {0, 2}, {0, 8}, {0, 4}, {10, 12}, {2, 12}, {8, 12}, {6, 12}, {4, 12}};
        ...
        for (int i = 0; i < edges.length; i++) {
            graph.addEdgeTemporarily(edges[i][0], edges[i][1], vertexTypes[i][0], vertexTypes[i][1],
                    null /* no weights */, edgeTypes[i]);
        }
    ...
    private void assertInsertions(Graph graph) throws Exception {
        ...
        for (int i = 0; i < expectedOutgoingAdjLists.length; i++) {
            ...
            for (int j = 0; j < expectedOutgoingAdjLists[i].length; j++) {
                expectedSortedAdjacencyList.add(expectedOutgoingAdjLists[i][j], expectedOutgoingEdgeTypes[i][j],
        ...
        for (int i = 0; i < expectedIncomingAdjLists.length; i++) {
            ...
            Assert.assertTrue("Testing BACKWARD vertex id: " + i, SortedAdjacencyList
                    .isSameAs(graph.getSortedAdjacencyList(i, Direction.BACKWARD, GraphVersion.PERMANENT),
                            expectedSortedAdjacencyList));
        }
    }
    ...
    public void testDeletionOfExistingEdges() throws Exception {
        // Create a graph.
        int[][] edges =
                {{0, 3} /* edgeId: 0 */, {0, 1}  /* edgeId: 1 */, {1, 3}  /* edgeId: 2 */, {1, 2}  /* edgeId: 3 */,
                        {4, 0}  /* edgeId: 4 */, {4, 1}  /* edgeId: 5 */};
        ...
        for (int i = 0; i < expectedOutgoingAdjLists.length; i++) {
            ...
            Assert.assertTrue("Testing FORWARD vertex id: " + i, SortedAdjacencyList
                    .isSameAs(graph.getSortedAdjacencyList(i, Direction.FORWARD, GraphVersion.PERMANENT), expected));
        }
        ...
        for (int i = 0; i < expectedIncomingAdjLists.length; i++) {
            ...
            Assert.assertTrue("Testing BACKWARD vertex id: " + i, SortedAdjacencyList
                    .isSameAs(graph.getSortedAdjacencyList(i, Direction.BACKWARD, GraphVersion.PERMANENT), expected));
        }
    }
    ...
    private Graph getGraphWithTemporaryChanges() {
        ...
        for (int i = 0; i < edges.length; i++) {
            graph.addEdgeTemporarily(edges[i][0], edges[i][1], fromVertexTypes[i], toVertexTypes[i], edgeTypes[i]);
        }
    ...
    public void testGetEdgesIteratorForPermanentGraph() throws Exception {
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY), new int[][]{{0, 1}, {11, 9}});
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.BACKWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY), new int[][]{{1, 0}, {9, 11}});
    ...
    public void testGetEdgesIteratorWithTypesForPermanentGraph() {
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{{0, 1}});
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.BACKWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{{1, 0}});
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{{0, 6}, {4, 6}, {5, 6}});
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.PERMANENT, Direction.BACKWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{{6, 0}, {6, 4}, {6, 5}});
    }
    ...
    public void testGetEdgesIteratorForMergedGraph() throws Exception {
        ...
        assertEdgesIterator(graph.getEdgesIterator(GraphVersion.MERGED, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY),
                new int[][]{{0, 6}, {2, 5}, {4, 6}, {5, 6}, {11, 9}});
        assertEdgesIterator(graph.getEdgesIterator(GraphVersion.MERGED, Direction.BACKWARD, TypeAndPropertyKeyStore.ANY,
                TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY),
                new int[][]{{5, 2}, {6, 0}, {6, 4}, {6, 5}, {9, 11}});
    ...
    public void testGetEdgesIteratorForDiffPlusGraph() throws Exception {
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.DIFF_PLUS, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY),
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.DIFF_PLUS, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY), new int[][]{});
    }
    ...
    public void testGetEdgesIteratorWithTypesForDiffPlusGraph() throws Exception {
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.DIFF_PLUS, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{{0, 6}, {2, 6}, {5, 6}, {4, 6}});
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.DIFF_PLUS, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, edgeType), new int[][]{});
    }
    ...
    public void testGetEdgesIteratorForDiffMinusGraph() throws Exception {
        ...
        assertEdgesIterator(
                graph.getEdgesIterator(GraphVersion.DIFF_MINUS, Direction.FORWARD, TypeAndPropertyKeyStore.ANY,
                        TypeAndPropertyKeyStore.ANY, TypeAndPropertyKeyStore.ANY), new int[][]{{2, 6}, {0, 1}});
        // Make the temporary changes permanent.
        graph.finalizeChanges();
