...
if [ $# -ne 19 ]; then
  echo "usage: $0 graph_name workload_name execution_mode number_queries connected_probability batch_size number_updates delete_probability number_runs sign range log_prefix drop_probdrop_type bloom_type java_max_memory bloomSize dropMinimum dropMaximum"
...
for ((i = 1; i <= ${number_runs}; i++)); do
  my_log=${global_log}/${graph_name}_${log_prefix}_${workload_name}_${exec_mode}_${number_queries}_${nb}_${batch_size}_${delete_probability}_${connected_probability}_${drop_prob}_${drop_type}_${bloom_type}_${dropMinimum}_${dropMaximum}_${java_max_mem}_${i}_"$(date +%Y%m%d-%H%M%S)"

  ./bench-init.sh ${my_log}

  echo "experiment-cspq-control -printdistances=false -reportingLevel=Debug  -graphFileToLoad=$load_file -connQueryFile=$connected_query_file  -disconnQueryFile=$disconnected_query_file -addEdgesFile=$add_file -deleteEdgesFile=$graph_delete_file -baseDir=$datasetDir -DropProbability=${drop_prob} -DropType=${drop_type} -BloomType=${bloom_type} -DropMinimum=${dropMinimum} -DropMaximum=${dropMaximum} -addDeleteSeed ${bloomSize}  -numQueries=${number_queries} -batchSize=${batch_size} -numBatches=${nb} -executorType=${execType} -deletionProbability=${delete_probability} -connectedQueryPercentage=${connected_probability} > ${my_log}.log"
  /usr/bin/time -v ./experiment-cspq-control -printdistances=false -reportingLevel=Debug -graphFileToLoad=$load_file -connQueryFile=$connected_query_file -printdistances="true"   -disconnQueryFile=$disconnected_query_file -addEdgesFile=$add_file -deleteEdgesFile=$graph_delete_file  -baseDir=$datasetDir -DropProbability=${drop_prob}  -DropType=${drop_type} -BloomType=${bloom_type}  -DropMinimum=${dropMinimum} -DropMaximum=${dropMaximum} -addDeleteSeed ${bloomSize}  -numQueries=${number_queries} -batchSize=${batch_size} -numBatches=${nb} -executorType=${execType} -deletionProbability=${delete_probability} -connectedQueryPercentage=${connected_probability} > ${my_log}.log 2>&1 
