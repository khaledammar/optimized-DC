...
if [ $# -ne 15 ]; then
  echo "usage: $0 graph_name workload_name execution_mode number_queries connected_probability batch_size number_updates delete_probability number_runs sign range log_prefix drop_probdrop_type bloom_type"
...
#Log directory:
global_log=/home/kammar/semih-new-project/2020_logs/
...
let execType="unw-baseline"
  ...
  case "${exec_mode}" in
    ...
    "Diff+JOT+DROP") execType="uni-w-diff-bfs-fake" ;;
...
for ((i = 1; i <= ${number_runs}; i++)); do
  ...
  echo "experiment-cspq-control -graphFileToLoad=$load_file -connQueryFile=$connected_query_file  -disconnQueryFile=$disconnected_query_file -addEdgesFile=$add_file -deleteEdgesFile=$graph_delete_file -baseDir=$datasetDir -DropProbability=${drop_prob} -DropType=${drop_type} -BloomType=${bloom_type}  -numQueries=${number_queries} -batchSize=${batch_size} -numBatches=${nb} -executorType=${execType} -deletionProbability=${delete_probability} -connectedQueryPercentage=${connected_probability} > ${my_log}.log"
  /usr/bin/time -v ./experiment-cspq-control-10G  -graphFileToLoad=$load_file -connQueryFile=$connected_query_file -printdistances="true"   -disconnQueryFile=$disconnected_query_file -addEdgesFile=$add_file -deleteEdgesFile=$graph_delete_file  -baseDir=$datasetDir -DropProbability=${drop_prob}  -DropType=${drop_type} -BloomType=${bloom_type}  -numQueries=${number_queries} -batchSize=${batch_size} -numBatches=${nb} -executorType=${execType} -deletionProbability=${delete_probability} -connectedQueryPercentage=${connected_probability} > ${my_log}.log 2>&1 
